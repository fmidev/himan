...
level::level() : itsType(kUnknownLevel), itsValue(kHPMissingValue), itsIndex(kHPMissingInt), itsName("") {}
level::level(HPLevelType theType, double theValue, const std::string& theName)
    : itsType(theType), itsValue(theValue), itsIndex(kHPMissingInt), itsName(theName)
{
}

level::level(HPLevelType theType, double theValue, int theIndex, const std::string& theName)
    : itsType(theType), itsValue(theValue), itsIndex(theIndex), itsName(theName)
{
}
...
level::level(const level& other)
    : itsType(other.itsType), itsValue(other.itsValue), itsIndex(other.itsIndex), itsName(other.itsName)
{
}
