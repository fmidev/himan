...
__global__ void print(double *d) { int idx = blockIdx.x * blockDim.x + threadIdx.x; printf("%d %f\n", idx, d[idx]); }
...
void simple_packed::Pack(double* d_arr, size_t N, cudaStream_t* stream)
{
	...
	coefficients.decimalScaleFactor = get_decimal_scale_fact(max, min, coefficients.bitsPerValue, coefficients.binaryScaleFactor);
