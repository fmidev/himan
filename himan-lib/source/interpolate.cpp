...
bool FromReducedGaussianCPU(info& base, info& source, matrix<double>& targetData)
{
	...
	double lonspan = (gg->BottomRight().X() - gg->TopLeft().X()); // longitude span of the whole gaussian area in degrees
	...
	else if (interpolationMethod == kBiLinear)
	{
		for (size_t i = 0; i < baseGrid->Data().Size(); i++)
		{
			...
			const int numUpperLongitudes = numOfLongitudesAlongParallels[static_cast<size_t> (static_cast<int> (floor(gg_y)))]; // number of longitudes for the upper parallel
			const int numLowerLongitudes = numOfLongitudesAlongParallels[static_cast<size_t> (static_cast<int> (ceil(gg_y)))]; // number of longitudes for the lower parallel
			assert(numUpperLongitudes > 0 && numLowerLongitudes > 0);
			
			const double dlon_upper = (lonspan / (numUpperLongitudes-1)); // longitude distance between two points for the upper parallel
			const double dlon_lower = (lonspan / (numLowerLongitudes-1)); // longitude distance between two points for the lower parallel

			double a_lon = gg->TopLeft().X() + floor((llpoint.X() - gg->TopLeft().X()) / dlon_upper) * dlon_upper; // longitude for point a
			...
			const double lat_upper = gg->TopLeft().Y() - floor((gg->TopLeft().Y() - llpoint.Y()) / dj) * dj; // latitude for a & b
			const double lat_lower = gg->TopLeft().Y() - ceil((gg->TopLeft().Y() - llpoint.Y()) / dj) * dj; // latitude for c & d
...
bool InterpolateArea(info& target, std::vector<info_t> sources, bool useCudaForInterpolation)
{
	...
	for (info_t& source : sources)
	{
		...
		matrix<double> targetData(target.Data().SizeX(), target.Data().SizeY(), target.Data().SizeZ(), target.Data().MissingValue(), target.Data().MissingValue());
		...
		switch (sourceType)
		{
			...
			case kStereographic:
			{
				switch (targetType)
				{
					...
					case kRotatedLatitudeLongitude:
					case kStereographic:
						useCudaForInterpolation ? InterpolateAreaGPU(target, *source, targetData) : InterpolateAreaCPU(target, *source, targetData);
						break;
			...
			case kReducedGaussian:
				switch (targetType)
				{
					...
					default:
						throw std::runtime_error("Unsupported target grid type for reduced gaussian: " + HPGridTypeToString.at(targetType));
				}
				break;
			default:
