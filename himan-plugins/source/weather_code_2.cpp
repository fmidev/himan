...
weather_code_2::weather_code_2()
{
	...
	itsLogger = std::unique_ptr<logger> (logger_factory::Instance()->GetLog("weather_code_2"));
}
...
void weather_code_2::Calculate(shared_ptr<info> myTargetInfo, unsigned short theThreadIndex)
{
	...
	unique_ptr<logger> myThreadedLogger = std::unique_ptr<logger> (logger_factory::Instance()->GetLog("weather_code_2Thread #" + boost::lexical_cast<string> (theThreadIndex)));
