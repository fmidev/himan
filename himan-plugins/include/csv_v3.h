...
struct can_not_open_file : base, with_file_name, with_errno
{
	void format_error_message() const
	{
		if (errno_value != 0)
			...
			std::snprintf(error_message_buffer, sizeof(error_message_buffer), "Can not open file \"%s\".", file_name);
	}
};
...
class LineReader
{
   private:
	...
	void init()
	{
		...
		data_begin = 0;
		data_end = static_cast<int>(std::fread(buffer, 1, 2 * block_len, file));

		// Ignore UTF-8 BOM
		if (data_end >= 3 && buffer[0] == '\xEF' && buffer[1] == '\xBB' && buffer[2] == '\xBF') data_begin = 3;

#ifndef CSV_IO_NO_THREAD
		if (data_end == 2 * block_len)
		{
			bytes_read = std::async(std::launch::async, [=]() -> int {
				return static_cast<int>(std::fread(buffer + 2 * block_len, 1, block_len, file));
			});
		}
#endif
	}
   ...
   public:
	...
	LineReader(const char *file_name, FILE *file) : file(file)
	{
		set_file_name(file_name);
		init();
	}

	LineReader(const std::string &file_name, FILE *file) : file(file)
	{
	...
	~LineReader()
	{
		...
		if (bytes_read.valid()) bytes_read.get();
#endif
...
struct header_missing : base, with_file_name
{
	void format_error_message() const
	{
		std::snprintf(error_message_buffer, sizeof(error_message_buffer), "Header missing in file \"%s\".", file_name);
	}
};
...
struct too_few_columns : base, with_file_name, with_file_line
{
	void format_error_message() const
	{
		std::snprintf(error_message_buffer, sizeof(error_message_buffer), "Too few columns in line %d in file \"%s\".",
		              file_line, file_name);
	}
};
...
struct too_many_columns : base, with_file_name, with_file_line
{
	void format_error_message() const
	{
		std::snprintf(error_message_buffer, sizeof(error_message_buffer), "Too many columns in line %d in file \"%s\".",
		              file_line, file_name);
	}
};
...
struct integer_must_be_positive : base, with_file_name, with_file_line, with_column_name, with_column_content
{
	void format_error_message() const
	{
		std::snprintf(error_message_buffer, sizeof(error_message_buffer),
...
struct integer_overflow : base, with_file_name, with_file_line, with_column_name, with_column_content
{
	void format_error_message() const
	{
		std::snprintf(error_message_buffer, sizeof(error_message_buffer),
...
struct integer_underflow : base, with_file_name, with_file_line, with_column_name, with_column_content
{
	void format_error_message() const
	{
		std::snprintf(error_message_buffer, sizeof(error_message_buffer),
...
struct invalid_single_character : base, with_file_name, with_file_line, with_column_name, with_column_content
{
	void format_error_message() const
	{
		std::snprintf(error_message_buffer, sizeof(error_message_buffer),
...
struct trim_chars
{
   ...
   public:
	...
	{
		while (is_trim_char(*str_begin, trim_char_list...) && str_begin != str_end) ++str_begin;
		while (is_trim_char(*(str_end - 1), trim_char_list...) && str_begin != str_end) --str_end;
		*str_end = '\0';
	}
};
...
struct no_comment
{
	static bool is_comment(const char *line) { return false; }
};
...
void parse(char *col, T &x)
{
	...
	static_assert(sizeof(T) != sizeof(T),
	              "Can not parse this type. Only buildin integrals, floats, char, char*, const char* and std::string "
	              "are supported");
}
}

template <unsigned column_count, class trim_policy = trim_chars<' ', '\t'>, class quote_policy = no_quote_escape<','>,
   ...
   public:
	...
	void read_header(ignore_column ignore_policy, ColNames... cols)
	{
		...
		try
		{
			...
			detail::parse_header_line<column_count, trim_policy, quote_policy>(line, col_order, column_names,
			                                                                   ignore_policy);
		}
